...
for project in run_list:
    for model_id in model_list:
        print(f"Running for project: {project} and model: {model_id}")
        # get_result(project=project)

        # load json file
        with open(f"./{project}_{model_id}_instruction_generation.json", "r") as f:
            data = json.load(f)
        overall_score = 0
        overall_key_score = 0
        ...
        with open(f"./eval/{project}_{model_id}_instruction_generation_summary.json", "w") as f:
            json.dump(summary, f)
